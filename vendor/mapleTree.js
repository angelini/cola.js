   ...
   if (typeof path === 'string') {
      ...
      if (path === '/') {
         ...
         paths.forEach(function (apath) {
           ...
           for (i = 0; i < portions.length; i+=1) portions[i] = createRoute(portions[i])  //returns {regexp:reg , params:[id1,id2,...]}
...
function pattern(toMatch) {
  ...
  regexps.forEach(function(aPath, ix) {
    regexps[ix] = createRoute(aPath, true).regexp //'true' tells regexp compiler to math till end => '$'
  })
